// A Bunker array is defined to be an array in which at least one odd number is immediately followed
...
public class BunkerArray {
    public static void main(String[] args){
        System.out.println(bunkerArray(new int[] {4, 9, 6, 7, 3}));
        System.out.println(bunkerArray(new int[] {4, 9, 6, 15, 21}));
        System.out.println(bunkerArray(new int[] {4, 9, 6, 16, 21, 3}));
    }

    public static int bunkerArray(int[] arr){
        boolean flag = false;
        for(int i = 0; i < arr.length; i++){
            if(arr[i] % 2 != 0){
                if(i+1 < arr.length && isPrime(arr[i+1]) == true){ // not to use i+ 1 <= arr.length (throws arrayIndexout of bounds exception)
                    flag = true;
                }
            }
        }
